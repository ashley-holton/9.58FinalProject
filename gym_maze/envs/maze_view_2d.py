...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None, maze_size=(30, 30),
                 screen_size=(600, 600), enable_render=True):
        ...
        y = random.randrange(0, 9, 9)
        if x == 0 or x == 9:
            y = random.randint(0, 9)
        self.__entrance = np.array([x, y])
    ...
    def reset_robot(self):
        ...
        self.__draw_robot(transparency=0)
        self.__entrance = self.get_new_entrance()
        self.__goal = self.get_new_goal()
        self.__robot = np.array(self.__entrance)# np.zeros(2, dtype=int)  # self.__entrance
